...
wss.on('connection', (ws) => {
  ...
  const player = { ws, id, name:'Player', avatar:null, x:0,y:1,z:0, ry:0, anim:'idle', world:null, partyCode:null, alive:true };
  ...
  send(ws, {
    t:'welcome',
    id,
    players: [...players.values()].filter(p=>p.id!==id).map(p=>({ id:p.id, name:p.name, avatar:p.avatar, x:p.x,y:p.y,z:p.z, ry:p.ry, anim:p.anim, world:p.world })),
  });
  ...
  ws.on('message', (raw) => {
    ...
    switch(msg.t){
      case 'hello': {
        ...
        broadcastAll({ t:'player_join', id, name:player.name, avatar:player.avatar, world:player.world }, id);
        break;
      }
      case 'presence': {
        ...
        if(msg.world && msg.world !== player.world){
          ...
          broadcastWorld(player.world, { t:'player_join_world', id, name:player.name, avatar:player.avatar, world:player.world }, id);
        }
        broadcastWorld(player.world, { t:'presence', id, x:player.x,y:player.y,z:player.z, ry:player.ry, anim:player.anim }, id);
        break;
      }
      case 'emote': {
