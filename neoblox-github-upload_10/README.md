...
- **Global voice room** — a separate, site-wide voice channel (🌐 in the party panel) that
  anyone online can join, independent of parties. Capped at 12 people at once (mesh WebRTC —
  every extra person adds a connection to everyone else already in, so this keeps it sane).
- **Lobby music** — a 🎵 button next to the party dock lets you upload your own audio file
  and play it as personal background music while you're in the lobby. Plays only for you
  (nothing is sent to the server or other players), pauses automatically when you head into
  a world/battle, and resumes when you're back.
- **Spotify "now playing" status** (optional) — connect your Spotify account from the 🎵
  panel and your currently-playing track shows up as a small 🎧 badge above your head and
  next to your name in the party list, visible to other players — like a Discord Spotify
  status. This is off by default and needs a one-time setup step by whoever runs this
  deployment (not per-player):
  1. Go to the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard) and
     create an app.
  2. Add a Redirect URI that exactly matches your deployed site's URL, e.g.
     `https://yourdomain.com/`.
  3. Copy the app's **Client ID** and paste it into `SPOTIFY_CLIENT_ID` near the top of the
     `// === SECTION:SPOTIFY` block in `public/index.html`, then redeploy.
  Until that's filled in, the "Connect" button just says "Not set up" and nothing else is
  affected — it only ever reads your currently-playing track, never touches playback.
