...
function partyRoster(code){
  ...
  return [...party.members].map(id => {
    const p = players.get(id);
    return p ? { id: p.id, name: p.name, avatar: p.avatar, inVoice: party.voice.has(id), nowPlaying: p.nowPlaying || null } : null;
  }).filter(Boolean);
}
...
wss.on('connection', (ws) => {
  ...
  const player = { ws, id, name:'Player', avatar:null, x:0,y:1,z:0, ry:0, anim:'idle', world:null, partyCode:null, alive:true, isAdmin:false, nowPlaying:null };
  ...
  send(ws, {
    t:'welcome',
    id,
    players: [...players.values()].filter(p=>p.id!==id).map(p=>({ id:p.id, name:p.name, avatar:p.avatar, x:p.x,y:p.y,z:p.z, ry:p.ry, anim:p.anim, world:p.world, isAdmin:p.isAdmin, nowPlaying:p.nowPlaying||null })),
    ...
    switch(msg.t){
      ...
      case 'presence': {
        ...
        if(msg.world && msg.world !== player.world){
          ...
          broadcastWorld(player.world, { t:'player_join_world', id, name:player.name, avatar:player.avatar, world:player.world, isAdmin:player.isAdmin, nowPlaying:player.nowPlaying||null }, id);
        }
      ...
      case 'now_playing': {
        // Self-reported "what I'm listening to" (e.g. from a connected Spotify account) —
        // relayed as a lightweight, on-change event, not folded into the per-frame presence
        // message. Trusted the same way name/avatar already are: display-only, not gameplay data.
        const t = msg.track;
        const track = (t && t.title) ? { title:String(t.title).slice(0,80), artist:String(t.artist||'').slice(0,80) } : null;
        player.nowPlaying = track;
        broadcastWorld(player.world, { t:'now_playing', id, track }, id);
        const party = partyOf(id);
        if(party) for(const mid of party.members) if(mid!==id) sendTo(mid, { t:'now_playing', id, track });
        break;
      }
      case 'gvoice_join': {
